...
Three things the source artwork cannot do on its own:
  ...
  * It is white on transparent, which vanishes against a light taskbar or a
    ...
    square.
  * It is a stacked lockup -- the bulb above the EMOTIV wordmark. An icon is
    square, and below about 128px the wordmark is a grey smudge that only
    muddies the silhouette. Below that again the rays around the bulb cost more
    than they are worth: they widen the artwork by half, which shrinks the bulb
    itself past legibility, so the small sizes carry the bulb alone.
...
The scanning board's copy of this script thickens its ink at 16px, and this one
deliberately does not: that artwork is thin dark strokes on white, which wash
out to grey, while this one is a dense outline with detail inside it. Dilating
it was measured at kernels from 3 to 15 and changed nothing worth having; at
the automatically-sized kernel it filled the bulb in solid. At 16px this mark
is a lightbulb-shaped mass of colour, which is enough to tell it apart from its
...
# Crop boxes into the source, measured from it rather than eyeballed: the
# alpha channel's own extents, and connected-component analysis to separate the
# bulb from the rays around it. The source is 1254x1254. The wordmark boundary
# is sharp -- the bulb's screw base is 254px wide at y=1080 and the wordmark
# 1035px at y=1090 -- so 1085 is the split.
LOCKUP = (81, 65, 1191, 1220)     # bulb, rays and the EMOTIV wordmark
MARK = (81, 65, 1191, 1085)       # bulb and rays, no wordmark
TIGHT = (274, 210, 975, 1085)     # bulb and screw base: the two largest components
