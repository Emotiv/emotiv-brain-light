; Inno Setup script for EMOTIV Brain Light (Windows).
;
; Wraps the PyInstaller onedir output into a single setup.exe, replacing the zip
; that people had to unpack by hand and then find the right .exe inside. Onedir
; rather than onefile on purpose: a onefile build unpacks itself to %TEMP% on
; every launch, which costs ten seconds or more of cold start.
;
...
;     iscc packaging/EmotivBrainLight.iss

#define AppName "EMOTIV Brain Light"
#define AppPublisher "EMOTIV"
#define AppExe "EMOTIV Brain Light.exe"
#define AppURL "https://github.com/Emotiv/emotiv-brain-light"
...
[Setup]
AppId={{68B3286D-09AA-4EC2-B61D-E7DC800415D2}
AppName={#AppName}
...
OutputDir=..
OutputBaseFilename=EMOTIV-Brain-Light-windows-x64-setup
...
[UninstallDelete]
; Settings, credentials and the light's address live in
; ~\.emotiv_brain_light\ and are deliberately left behind on uninstall, so a
; reinstall does not send the user back through first-run setup. Only what the
...
begin
  ...
  if not WebView2Installed() then
    MsgBox('The Microsoft Edge WebView2 Runtime does not appear to be installed.'
      + #13#10#13#10
      + 'EMOTIV Brain Light draws its window with WebView2. Without it the app'
